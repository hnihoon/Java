...
public class GuiMain extends JFrame implements ActionListener {
	...
	private void init() {
		...
		bt2 = new JButton("단어추가");
		bt2.setBounds(150, 200, 90, 40);
		
		bt3 = new JButton("단어추가");
		bt3.setBounds(250, 200, 90, 40);
		
		bt4 = new JButton("단어추가");
	...
	public void actionPerformed(ActionEvent e) { 	//이벤트(사건) 일어난 객체정보가 인자입니다.
		if(e.getSource()==bt1) {
			JOptionPane.showConfirmDialog(this, "단어추가 입니다.");
			
			la5.setText(tf1.getText() + ":" + tf2.getText()
			+ "(" + jlist.getSelectedValue() + ")");
			
		} else if(e.getSource()==bt2) {
			JOptionPane.showConfirmDialog(this, "파일저장 입니다.");
		} else if(e.getSource()==bt3) {
			if(tf3.getText().length()==0) {
				JOptionPane.showConfirmDialog(this, "단어검색 입니다.");
			} else {
				new TestKiosk();
			}
		} else if(e.getSource()==bt4) {
			JOptionPane.showConfirmDialog(this, "프로그램 종료 입니다.");
