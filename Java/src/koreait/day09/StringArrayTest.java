...
//		String 클래스의 메소드 실행 " charAt(0), subString(2,5) 를 각 배열 요소에 문자열로 실행합니다.
		for(int i=0; i<strArray.length; i++) {
		...
		for(int i=0; i<names.length; i++) {
			System.out.println("i=" + i + " ,문자열=" + names[i]);  //String 은 클래스, 문자열은 객체 -> names 배열 , names[i] 둘다 참조(주소)값을 저장합니다.
		}
//		System.out.println("charAt(0) : " + names[0].charAt(0) + 
//				", subString(2,5) : "  + names[0].substring(2,3));
		//문자열이 null 일때 charAt 등 메소드 실행은 오류 발생
		//오류의 명칭(원인) NullPointerException
