...
public class ObjectArgMethod {
	//int 배열의 정수값 중에서 최대값/최소값 구하는 매소드를 만듭니다. -참조값을 인자로 전달하는 연습합니다.
	public static void main(String[] args) {
		...
		int max; 	//max 초기값 :1) 배열의 인덱스 0번값 		2) 비교될 다른 값들보다 작은값
		...
		min=999;	
		for(int i=0; i<arr.length; i++) {
			if(arr[i]<min)		//최소값은 다른 값들보다 작은값이 되어야 하므로, 그렇지 않을경우
	...
	//메소드 작성으로 구현해봅니다. : 최대값/최서값으로 구하기 위해 다룰 데이터는 배열에 있습니다. -> 배열 변수는 참조 변수.
	...
	static int arrayMinValue(int[] array) {
		int min=999;	
		for(int i=0; i<array.length; i++) {
			if(array[i]<min) 		//최소값은 다른 값들보다 작은값이 되어야 하므로, 그렇지 않을경우
