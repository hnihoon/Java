...
public class StringSplitTest {
	...
	public static void main(String[] args) {
		...
		List<String> list;
		list = new ArrayList<>();	//새로운 리스트 생성. 데이터X
		for(String data : result)
			list.add(data);
		...
		//result 배열을 리스트 자료구조로 변환(리스트 개겣가 결과로 생성됩니다.)
		list = Arrays.asList(result);	//이 클래스에서 사용할 수 있는 가변 리스트 생성(인자는 변환된 리스트)
				//이 변환은 고정길이 리스트를 생성합니다. 새로운 데이터를 추가 할 수 없다
...
//		list = new ArrayList<>(result);	//배열은 Collection
		list.add("나나");
		System.out.println("변환된 리스트에 항목추가 : " + list);
		
		//Collection 인터페이스 : 데이터 저장하는 자료구조. List, Set 인터페이스 하위 인터페이스 
