...
//		Exception 을 예상하고 프로그램이 중단되지 않도록 프로그램 구현을 해야합니다. 
		
		//Exception 예시 4 : Wrapper 클래스(Short,Integer,Double,Long...)에는 문자열을 기본형타입으로 변환하는 메소드가 있습니다.
...
//		nums[5] = 100;		//실행했을 때 ArrayIndexOutOfBoundsException 이 발생합니다. -> 인덱스 0~4
		
		//Exception 예시1
		String message = null;
		
		char temp = message.charAt(0);	//message 문자열에서 첫번째(0) 글자 1개 가져옵니다.
		//실행했을때 NullPointerException 이 발생합니다. -> 이유 : message 참조객체 null이기 때문이빈다.
